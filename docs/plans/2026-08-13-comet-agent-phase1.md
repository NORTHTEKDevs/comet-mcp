...
`%USERPROFILE%\projects\active\ghost\target\release\ghost-mcp.exe`, but the REGISTERED/canonical
binary is `%USERPROFILE%\.local\bin\ghost-mcp.exe`. Change the default to the `.local\bin` path.

...
**Tech Stack:** TypeScript 5.6, Node 20+, `@modelcontextprotocol/sdk`, `zod`, `vitest`. No external runtime deps beyond MCP SDK + zod. Ghost MCP is the only system dependency (already installed at `%USERPROFILE%\projects\active\ghost\target\release\ghost-mcp.exe`).
...
```bash
cd $HOME/projects/active/comet-mcp
npm install
```
...
**Files:** none modified in repo; modifies `%USERPROFILE%\.claude.json` via CLI.
